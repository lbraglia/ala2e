source_dir <- 'dta'
files <- tools::file_path_sans_ext(list.files(source_dir))

df_list <- lapply(files, function(f){
    foreign::read.dta(sprintf("%s/%s.dta", source_dir, f))
})

export <- function(x, n) {# x is the dataset, n its name
  ## now save in proper name and export
  eval(parse(text = sprintf("%s <- x", n)))
  save(list = n,
       file = sprintf("../data/%s.rda", n),
       compress = "bzip2")
}
Map(export, df_list, as.list(files))
